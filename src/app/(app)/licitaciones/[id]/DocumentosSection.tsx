...
import { uploadDocumento, deleteDocumento, getDocumentoUrl } from "../actions";
import { TIPO_DOCUMENTO_LABELS, type Documento, type TipoDocumento } from "@/lib/types";
...
export function DocumentosSection({
  ...
  function handleUpload(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        await uploadDocumento(licitacionId, formData);
  ...
  function handleDelete(documento: Documento) {
    if (!confirm(`¿Eliminar "${documento.nombre}"?`)) return;
    setError(null);
    startTransition(async () => {
      try {
        await deleteDocumento(licitacionId, documento.id, documento.storage_path);
      } catch (e) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <h2 className="mb-4 font-medium text-slate-900">Documentos</h2>
      ...
      {documentos.length === 0 ? (
        <p className="text-sm text-slate-400">No hay documentos asociados a esta licitación.</p>
      ) : (
              ...
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400">
                    {TIPO_DOCUMENTO_LABELS[doc.tipo as TipoDocumento]} · {formatBytes(doc.tamano_bytes)} ·{" "}
